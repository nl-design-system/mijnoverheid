...
const parseColor = (color) => {
  ...
  if (color.trim() !== '') {
    ...
    if (!colorLAB) {
      ...
      try {
        const [l, a, b] = Color(color).lab().color;
        colorLAB = { l, a, b };
      } catch (e) {
        colorLAB = null;
      }
...
export const ColorSearch = ({ tokens }) => {
  ...
  useEffect(() => {
    setResults(filterColorTokens(tokens, query, 25));
  }, [query]);
  ...
  return (
    <div className="color-search">
      ...
      {query ? Object.values(results).length > 0 ? <ColorTable tokens={results}></ColorTable> : <p>No results</p> : ''}
    </div>
  );
};
